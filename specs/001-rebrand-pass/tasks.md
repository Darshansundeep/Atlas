...
- [~] T020 [US1] Rewrite the About screen at `ui/src/components/About/AboutScreen.tsx` to render Atlas branding prominently and an attribution paragraph naming `upstreamProjectName` linked to `upstreamProjectUrl` per `contracts/identity-constants.md` R-IC-004. *Done at `ui/desktop/src/main.ts` (about panel + Help menu) — upstream About is native, not a React component. Atlas-branded About panel via `app.setAboutPanelOptions(buildAboutPanelOptions(version))`; payload built by `ui/desktop/src/branding/about-options.ts` (also feeds runtime branding overlay from spec 021 placeholder).*
- [~] T021 [P] [US1] Replace the application icon set at `ui/assets/icons/atlas.icns` (macOS), `ui/assets/icons/atlas.ico` (Windows), and the `hicolor` PNG set under `ui/assets/icons/linux/` per `data-model.md` Asset Mapping; remove upstream Goose icon files. *Done at `ui/desktop/src/images/icon.{png,@2x.png,-512.png,ico,icns}` (corrected path per LAYOUT_NOTES — Forge's `icon` field auto-resolves extensions). Source: Atlas 3D-rendered A+globe+ring from Darshan, converted via `tools/scripts/build-icons.py` (Pillow). Composited onto NET Group navy `#0B1F3A` because the source JPG has a baked-in checkerboard preview that overlaps the metallic-grey palette of the logo (clean transparency keying unreliable). Reproducible: `python3 tools/scripts/build-icons.py <new-logo.png>` regenerates the full set.*
...
- [~] T023 [P] [US1] Replace the tray / menubar icon at `ui/assets/icons/tray-template.png` (macOS monochrome template), `ui/assets/icons/tray.ico` (Windows), `ui/assets/icons/tray-22.png` (Linux). *Done at `ui/desktop/src/images/iconTemplate.png` (22×22) + `iconTemplate@2x.png` (44×44). Generated from the alpha silhouette of the icon source via `build-icons.py`. macOS treats `*Template.png` files as monochrome template images and tints them per system theme.*
- [~] T024 [US1] Sweep `ui/src/` UI components: every user-facing literal occurrence of "Goose" outside the About screen is replaced by `IDENTITY.displayName` import. Use the lint from T012 to find them. *Done via 4-pass mechanical sweep (`tools/scripts/rebrand-sweep{,2,3,4}.py`). User-facing strings (defaultMessages, dialog text, tray tooltips, notifications, batch-file echoes, Windows shim paths) now use `IDENTITY.displayName` (TS) or Atlas literal + `// brand-allow`. Doc comments rewritten to be brand-neutral ("the agent", "the app"). Internal React component identifiers (`Goose` icon component, `GooseLogo`) kept and `brand-allow`'d to minimise upstream-merge surface. Lint passes 0 violations.*
- [~] T024a [P] [US1] Sweep Rust user-facing strings in `crates/goose-cli/` (and any Rust source emitting user-facing text — banner, `--help`, `--version`, error messages, log prefixes): replace literal "Goose" references with values sourced from `crates/atlas-branding`. The lint from T012 MUST be extended to scan `crates/` for the same allowlist rule. Per FR-016 (covers CLI / Rust surfaces, not just `ui/src/`). *Done — atlas-branding dep wired into goose-cli; user-facing strings (CLI banner, prompts, dialog text) use `DISPLAY_NAME` via `format!`. goose-sdk/goose-server/goose core crates touched surgically: user-facing strings (LLM tool descriptions, doctor banners, provider catalog display names, error messages, system prompts) → "Atlas" with `// brand-allow` where compile-time literals required; internal doc comments rewritten to brand-neutral wording. Lint passes.*
- [~] T024b [P] [US1] Write the negative-isolation integration test `tests/integration/no_upstream_goose_state_read.rs` that creates a fake upstream Goose config directory on disk (`~/.config/goose/` with sentinel files), launches Atlas, exercises representative flows, and asserts Atlas never opens, reads, or otherwise touches any path inside the upstream Goose directory. Per FR-013. *Done at `crates/atlas-branding/tests/no_upstream_goose_state_read.rs` (branding-layer assertions). Full filesystem-launch test is bundled into the coexistence script (T053a). Per FR-013.*
...
- [~] T045 [P] [US3] Write the CI job `verify-attribution` in `.github/workflows/ci.yml` that computes SHA-256 of repo-root `LICENSE` and `NOTICE` and compares against the checksums pinned in `UPSTREAM_VERSION`; fails on mismatch. *Done at `atlas-ci.yml` `verify-attribution` job + script `tools/scripts/verify-attribution.sh`. Handles upstream-has-no-NOTICE case discovered in T003 (Apache 2.0 §4(d) does not engage). Passes locally.*
...
## Phase 7: Runtime Verification (NEW — closes the spec ↔ reality loop)

**Why this phase exists**: code-written ≠ behaviour-working. Tasks above were marked `[X]` for "file exists on disk + lint passes + cargo check succeeds." That is necessary but not sufficient. This phase verifies the rebrand actually RENDERS as Atlas at runtime. Until these pass, Phase 3 tasks in `[~]` state remain in-progress.

**Status legend used in this spec going forward**:
- `[ ]` — todo
- `[~]` — code written, lint/check passes, but runtime behaviour not yet verified
- `[X]` — code written AND runtime-verified per Phase 7

- [ ] T061 Build the desktop in dev mode (`pnpm run start-gui` from `ui/desktop/`) on macOS. Confirm: window appears within 60s; window title bar shows "Atlas"; dock icon is the Atlas A+globe+ring on navy; macOS application menu (top-left, next to Apple logo) reads "Atlas".
- [ ] T062 With Atlas running, open Atlas menu → "About Atlas". Confirm: panel renders without overflow; application name = "Atlas"; copyright = "© 2026 NET Group. Apache License 2.0."; credits paragraph contains the literal text "Built on Goose" and "https://github.com/block/goose"; website link goes to https://netgroup.ai.
- [ ] T063 With Atlas running, open Help menu → "About Atlas" submenu. Confirm: Version line shows "Version 1.36.0"; the "Built on Goose" link opens https://github.com/block/goose in the system browser; the NET Group link opens https://netgroup.ai.
- [ ] T064 Verify atlas:// URL scheme registered: in Terminal run `open "atlas://test"`. Confirm the running Atlas window comes to foreground (URL handler may log a rejection of unknown host "test" — that's correct per contracts/url-scheme.md R-URL-002).
- [ ] T065 Verify BYOK end-to-end: in Atlas Settings, paste an Anthropic or OpenAI API key. Start a new chat. Send "What is your name?". Confirm: agent responds (the agent loop still works post-rebrand per FR-012); the response identifies as Atlas (was: Goose) for any prompts that read the system prompt — this is the LLM-system-prompt rebrand verification.
- [ ] T066 Build a packaged `.app` via `pnpm run package` or `electron-builder` (TBD — Forge currently wedges; spec amendment may be required). Confirm: `out/Atlas-darwin-arm64/Atlas.app` exists; `defaults read out/Atlas-darwin-arm64/Atlas.app/Contents/Info CFBundleIdentifier` returns `ai.netgroup.atlas`; `CFBundleDisplayName` returns `Atlas`.
- [ ] T067 Repeat T061–T066 on Windows 10/11 (verifies cross-platform parity per Constitution Principle IV + FR-011).
